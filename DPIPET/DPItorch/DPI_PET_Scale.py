...
parser.add_argument("--sinogram_path", default="../dataset/brain64_tumor_dose_output/brain64_tumor_FDG_K1_40min.mat",
...
parser.add_argument("--ri_path", default="../dataset/brain64_tumor_dose_output/brain64_tumor_FDG_K1_40min_ri.mat", 
...
#                     help="探测器效率扰动路径")
parser.add_argument("--ytrue_path", default="../dataset/brain64_tumor_dose_output/brain64_tumor_FDG_K1_40min_ytrue.mat", 
                    help="无噪声真实投影路径 (用于验证)")
parser.add_argument("--scale_path", default="../dataset/brain64_tumor_dose_output/brain64_tumor_FDG_K1_40min_scale.mat",
                    help="剂量水平/缩放因子路径 (替换ci, .mat 文件中的键名应为 'scale')")
...
if args.image_size == 64:
    ...
    if "../dataset/brain128_tumor" in args.activity_path:
        # args.activity_path = args.activity_path.replace("brain128_tumor", "brain64_tumor") # activity_path通常单独指定，不强制改
        args.sinogram_path = args.sinogram_path.replace("brain128_tumor_dose_output", "brain64_tumor_dose_output").replace("brain128", "brain64")
        args.ri_path = args.ri_path.replace("brain128_tumor_dose_output", "brain64_tumor_dose_output").replace("brain128", "brain64")
        args.ytrue_path = args.ytrue_path.replace("brain128_tumor_dose_output", "brain64_tumor_dose_output").replace("brain128", "brain64")
        args.scale_path = args.scale_path.replace("brain128_tumor_dose_output", "brain64_tumor_dose_output").replace("brain128", "brain64")
    print(f"[INFO] 使用64x64数据集路径和{gmat_file} (部分路径可能来自dose_output)")
elif args.image_size == 128:
    gmat_file = "G_system_matrix.mat" # 通常128尺寸的系统矩阵文件名不含尺寸
    # 确保路径是128版本
    if "brain64_tumor" in args.activity_path: # activity_path 保持不变
        # args.activity_path = args.activity_path.replace("brain64_tumor", "brain128_tumor") # activity_path通常单独指定，不强制改
        args.sinogram_path = args.sinogram_path.replace("brain64_tumor_dose_output", "brain128_tumor_dose_output").replace("brain64", "brain128")
        args.ri_path = args.ri_path.replace("brain64_tumor_dose_output", "brain128_tumor_dose_output").replace("brain64", "brain128")
        args.ytrue_path = args.ytrue_path.replace("brain64_tumor_dose_output", "brain128_tumor_dose_output").replace("brain64", "brain128")
        args.scale_path = args.scale_path.replace("brain64_tumor_dose_output", "brain128_tumor_dose_output").replace("brain64", "brain128")
    print(f"[INFO] 使用128x128数据集路径和{gmat_file} (部分路径可能来自dose_output)")
else:
...
# 加载 scale (剂量水平/缩放因子)
scale_data_raw = load_mat(args.scale_path, 'scale') # 假设 .mat 文件中的键为 'scale'

# 检查数据的维度
if scale_data_raw.ndim == 4:
    scale_slice = scale_data_raw[:, :, roi, frame]
elif scale_data_raw.ndim == 2:
    # 假设如果scale是2D, 它已经是正确的 (nb, na) 形状，或者需要调整
    # 这里我们假设它已经是 (nb, na) 对应当前 roi 和 frame
    # 如果不是, 可能需要进一步处理，例如基于文件名中的信息来验证或调整
    print(f"[INFO] 'scale' 数据在 {args.scale_path} 中是2维的，直接使用。")
    scale_slice = scale_data_raw
else:
    raise ValueError(f"'scale' 数据在 {args.scale_path} 中的维度 ({scale_data_raw.ndim}D) 不支持。"
                     f"期望是2D或4D。")

scale_vec = torch.tensor(scale_slice.T.flatten(), dtype=torch.float32, device=device)
print("✔ 缩放因子 scale_vec shape:", scale_vec.shape)

# 验证 ytrue (可选但推荐)
# 将真实活度图展平以匹配系统矩阵的输入
act_gt_vec = torch.tensor(act_gt.flatten(order='F'), dtype=torch.float32, device=A_torch.device) 

# 计算 y_calc_from_gt = scale * (A @ act_gt) + ri
...
# 分两步验证 - 第一步：验证 ytrue ≈ scale * (A @ act_gt)，不考虑ri
# y_calc_no_ri = ci_vec * proj_gt_no_ci_ri 
y_calc_no_ri = scale_vec * proj_gt_no_ci_ri 
mse_ytrue_no_ri = torch.mean((ytrue_vec - y_calc_no_ri)**2)
print(f"[验证第一步] ytrue与(scale*A@act_gt)的MSE: {mse_ytrue_no_ri.item():.4e}")

# 第二步：验证完整模型 y ≈ scale * (A @ act_gt) + ri
# y_calc_from_gt = ci_vec * proj_gt_no_ci_ri + ri_vec
y_calc_from_gt = scale_vec * proj_gt_no_ci_ri + ri_vec
mse_ytrue_validation = torch.mean((ytrue_vec - y_calc_from_gt)**2)
print(f"[验证第二步] ytrue与完整模型((scale*A@act_gt)+ri)的MSE: {mse_ytrue_validation.item():.4e}")
...
if mse_ytrue_no_ri.item() > 1e-5:  # 第一步验证阈值
    print(f"[警告] ytrue与scale*(A@act_gt)差异较大，系统矩阵A或缩放因子scale可能有问题！")
    ...
    # 2. 检查scale和ri的统计信息
    print(f"scale形状: {scale_vec.shape}, 最小值: {scale_vec.min().item():.4f}, 最大值: {scale_vec.max().item():.4f}, 均值: {scale_vec.mean().item():.4f}")
    ...
    print(f"ytrue与scale*(A@act_gt)之间的相对误差: {rel_error_no_ri:.4%}")
    ...
    try:
        ...
        plt.title('计算投影 (scale*(A@act_gt))')
        plt.colorbar()
        
        plt.subplot(144)
        plt.imshow(y_calc_with_ri_reshaped, cmap='hot')
        plt.title('计算投影 (scale*(A@act_gt)+ri)')
        plt.colorbar()
        ...
        plt.title('ytrue与scale*(A@act_gt)的差异')
        plt.colorbar()
    ...
    print("2. scale或ytrue文件与活度图不对应")
    print("3. ytrue可能是另一种方式生成的，而非简单的scale*(A@act_gt)")
...
if args.model_form == 'realnvp':
    ...
    n_channel = 1 if (args.image_size == 64) else 1
    z_shapes = glow_model.calc_z_shapes(n_channel, H, args.n_flow, args.n_block)
    Gnet = glow_model.Glow(n_channel, args.n_flow, args.n_block, True, True).to(device)
...
# logdet 权重调整:
# 移除1e-6因子，使args.logdet的调整更直接影响logdet_w
# 如果args.logdet设为1，logdet_w将是 1 / (nz_ratio * N_pix)
# 用户应相应调整args.logdet的值 (例如 1e-2, 1e-3, 或根据经验调整)
# 为了使当前默认值args.logdet=1.0产生合理的占比，增加额外的缩放因子
logdet_w = args.logdet * 1e-4 / (nz_ratio * N_pix) 

# TV 权重调整 - 大幅降低TV权重，以确保data loss占主导地位
# 原始计算方式: tv_w = args.tv / (mean_act_gt * W)
# 为了使当前默认值args.tv=1.0产生合理的占比，进一步调整分母
...
best_epoch = -1
no_improve_count = 0
early_stopping_patience = 20  # 连续50次检查（50*print_interval epochs）没有改善则停止
...
for epoch in trange(args.n_epoch, desc="Training Epoch"):
    ...
    if A_torch.device.type != 'cuda':
        ...
        for b in range(args.n_batch):
            ...
            lam[b] = scale_vec * proj_no_ci_ri + ri_vec
    else:
        # 系统矩阵在GPU上，直接计算
        ...
        lam = scale_vec.unsqueeze(0) * proj_no_ci_ri_batch + ri_vec.unsqueeze(0) # scale*(A·x) + ri for batch
    ...
    if epoch % print_interval == 0:
        ...
        with torch.no_grad():
            ...
            if current_total_loss < best_loss:
                ...
                no_improve_count += 1
                # print(f"[模型状态] 当前Loss: {current_total_loss:.6e} (vs最佳: {best_loss:.6e} @Epoch {best_epoch}), "
                #       f"{no_improve_count}/{early_stopping_patience} 次未改进")
